...
/************************************************************************/
/* 从输入的若干组(x, y)经纬度形式的GPS数据点中，计算拐弯的点
 *
 * 最开始的第0、1个点作为作为种子点，起点为A  终点为B，构成种子向量，作为当前向量
 *
 * 计算当前向量 和 当前向量的终点与下一个点C 构成的向量的 夹角(AB - BC)
 * 如果夹角超过阈值，则认为此点为疑似拐点，则接着计算(AB - BD,BE,BF)是否仍然
 * 三个夹角仍然超过阈值，如果超过则认为C点为拐点，更新起点为C，终点为F
 * 如果遇到第一个夹角不超过阈值，则更新终点为该点，起点不变
/************************************************************************/
#include "stdafx.h"
#include <iostream>
#include <math.h>
